...
namespace ZXing.Mobile
{
	public sealed partial class ZXingScannerControl : UserControl, IScannerView, IScannerSessionHost, IDisposable
	{
		...
		public async Task StartScanningAsync(Action<ZXing.Result> scanCallback, MobileBarcodeScanningOptions options = null)
		{
            ...
            ////capturedImage.Source = mediaCapture; ////TODO
            capturedImage.FlowDirection = mirroringPreview ? FlowDirection.RightToLeft : FlowDirection.LeftToRight;
            ...
            capturedImage.Stretch = Stretch.UniformToFill;
		...
		protected override void OnPointerPressed(PointerRoutedEventArgs e)
		{
			...
			var pt = e.GetCurrentPoint(capturedImage);
				...
				try
				{
					if (roiControl.AutoFocusSupported && roiControl.MaxRegions > 0)
					{
						if (useCoordinates)
						{
							...
							var previewRect = GetPreviewStreamRectInControl(previewEncodingProperties, capturedImage);
		...
		/// <returns></returns>
		static Rect GetPreviewStreamRectInControl(Size previewResolution, Image previewControl)
		{
			var result = new Rect();
